...
public final class SurvivalistEssentialsWorld {
    ...
    private static final Map<ResourceLocation, Item> ALL_ITEMS = new LinkedHashMap<>();
    ...
    public static void initItems(BiConsumer<Item, ResourceLocation> consumer) {
        makeItem("andesite_loose_rock", ANDESITE_LOOSE_ROCK);
        makeItem("diorite_loose_rock", DIORITE_LOOSE_ROCK);
        makeItem("granite_loose_rock", GRANITE_LOOSE_ROCK);
        makeItem("stone_loose_rock", STONE_LOOSE_ROCK);
        makeItem("sandstone_loose_rock", SANDSTONE_LOOSE_ROCK);
        makeItem("red_sandstone_loose_rock", RED_SANDSTONE_LOOSE_ROCK);
        makeItem("rock_stone_block", ROCK_STONE_BLOCK);

        for (Map.Entry<ResourceLocation, Item> entry : ALL_ITEMS.entrySet()) {
            consumer.accept(entry.getValue(), entry.getKey());
        }
    }
    ...
    private static Block makeBlock(String name) {
        ...
        ALL_BLOCKS.put(new ResourceLocation(SurvivalistEssentials.MODID, name), block);

        return block;
    }

    private static void makeItem(String name, Block block) {
        makeItem(name, new BlockItem(block, new Item.Properties()));

        if (name.contains("rock_stone_block")) {
            ROCK_STONE = makeItem("rock_stone", new RockStone(block, new Item.Properties()));
        }
    }

    private static Item makeItem(String name, Item item) {
        ResourceLocation loc = new ResourceLocation(SurvivalistEssentials.MODID, name);

        ALL_ITEMS.put(loc, item);
